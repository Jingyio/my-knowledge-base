...
static char g_kheap[KERNEL_HEAP_SIZE];
static heap_chunk_t chunk_start, chunk_end;
static unsigned int available_bytes = 0;

static void kheap_init(void)
{
        chunk_start.next = (heap_chunk_t *)g_kheap;
        chunk_start.chunk_size = 0;
        chunk_start.prev = 0;

        chunk_end.prev = (heap_chunk_t*)g_kheap;
        chunk_end.next = 0;
        chunk_end.chunk_size = KERNEL_HEAP_SIZE;

        ((heap_chunk_t*)g_kheap)->prev = &chunk_start;
        ((heap_chunk_t*)g_kheap)->next = &chunk_end;
        ((heap_chunk_t*)g_kheap)->chunk_size = KERNEL_HEAP_SIZE;

        available_bytes = KERNEL_HEAP_SIZE;
}

char* kmalloc(unsigned int alloc_size)
{
